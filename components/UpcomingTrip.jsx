    ...
    const fetchData = async () => {
      try {
        ...
        const enrichedTrips = tripData.map(trip => {  
          const matchedPackage = packageData.find(pkg => decodeHtml(pkg.packageName) === decodeHtml(trip.tripName));
          return {
            ...trip,
...
function decodeHtml(html) {
  ...
  >
    {trips.map((trip, idx) => (
      <div className={styles["trip-card"]} key={idx}>
        ...
        <div className={styles["trip-content"]}>
          <h3 className={styles["trip-name"]}>{decodeHtml(trip.tripName)}</h3>
          {trip.travelDate && (
          ...
          {trip.packageDetails?.price && (
            <p className={styles["trip-price"]}>₹{trip.packageDetails.price}</p>
          )}
          {trip.packageDetails?.duration && (
            <p className={styles["trip-duration"]}>{trip.packageDetails.duration}</p>
          )}
        </div>
        {!trip.packageDetails && (
  ...
  <p className={styles["no-trips-message"]}>No upcoming trips available.</p>
)}
    </div>
  );
};
