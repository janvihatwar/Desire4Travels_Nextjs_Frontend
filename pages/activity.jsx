...
export default function Activity() {
  ...
  const handleSubmit = async () => {
    if (!number) return alert('Please enter your mobile number.');
    if (!selectedActivity) return alert('No activity selected.');
    ...
    try {
      const res = await fetch('https://desire4travels-1.onrender.com/activity-callback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ number, activityName: selectedActivity }),
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        ...
        <meta property="og:description" content="Explore thrilling adventure activities with Desire4Travels including trekking, paragliding, rafting, scuba diving, and more. Safe, guided, and unforgettable experiences await you." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://www.desire4travels.com/activity" />
      </Head>
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {activities.map((activity) => (
            ...
            >
              <Link href={`/activity/${activity.slug}`} passHref>
                <div className="h-48 bg-gray-300 overflow-hidden cursor-pointer">
                  <img
                    src={activity.image}
                    alt={activity.title}
                    className="w-full h-full object-cover"
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-xl text-center">
            {!submitted ? (
              <>
                ...
                <h3 className="text-gray-700 mb-4 text-sm italic">
                  For: <span className="font-medium text-blue-700">{selectedActivity}</span>
                </h3>
                <input
                  type="tel"
                  value={number}
